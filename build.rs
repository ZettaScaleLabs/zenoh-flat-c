...
fn generate_flat_bindings() -> PathBuf {
    ...
    cbindgen = cbindgen.opaque_owned(pq!(ZZBytes), pq!(z_zbytes_t));
    ...
    cbindgen = cbindgen.opaque_owned(pq!(ZSample), pq!(z_sample_t));
    ...
    cbindgen = cbindgen.opaque_owned(pq!(ZReply), pq!(z_reply_t));
    cbindgen = cbindgen.opaque_owned(pq!(ZQuery), pq!(z_query_t));
    cbindgen = cbindgen.opaque_owned(pq!(ZHello), pq!(z_hello_t));

    // `ZTimestamp` (`uhlc::Timestamp`) is a small `Copy` POD that owns no external
    // data — inline it by value as `opaque_data`. There is no per-message box to
    ...
    cbindgen = cbindgen.opaque_data(pq!(ZTimestamp), pq!(z_timestamp_t));
