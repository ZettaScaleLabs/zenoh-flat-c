...
/// silently accept. Use [`try_get_project_root`] for a non-panicking variant.
pub fn get_project_root() -> PathBuf {
    try_get_project_root().unwrap_or_else(|e| panic!("{e}"))
}

/// Non-panicking [`get_project_root`]: returns `Err` (with guidance) when the
/// crate is a regular unpatched dependency and the workspace root is unknown.
pub fn try_get_project_root() -> Result<PathBuf, &'static str> {
    let project_root = env!("PROJECT_ROOT");
    if project_root.is_empty() {
        Err(NOT_IN_WORKSPACE)
    } else {
