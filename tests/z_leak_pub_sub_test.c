...
// delivered through the owned-sample handler, and exercises the full
// allocate/drop lifecycle (run under valgrind for a leak check).
//
#include <stdio.h>
#include <string.h>
#include <unistd.h>
...
int main(void) {
    ...
    z_publisher_t* pub = z_session_declare_publisher(pub_session, pub_ke, Block, Data, false, Reliable, NULL);
#else
    z_publisher_t* pub = z_session_declare_publisher(pub_session, pub_ke, Block, Data, false, NULL);  // consumes pub_ke
#endif
    assert(pub != NULL);
