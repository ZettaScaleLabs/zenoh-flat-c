...
// NOTE: `crate::z_timestamp_t` is declared `opaque_data_struct` (plain `Copy` POD,
// owns no external data), so it needs no `Gravestone` impl — only the
// autogenerated `Transmute`.
